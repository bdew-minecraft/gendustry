...
object EjectCover extends BaseItem("eject_cover") with ItemCover {
  ...
  override def tickCover(te: TileCoverable, side: EnumFacing, coverStack: ItemStack): Unit = {
    if (te.getWorldObject.getTotalWorldTime % 20 == 0) {
      for (target <- ItemHelper.getItemHandler(te.getWorldObject, te.getPos.offset(side), side.getOpposite))
        ItemHelper.pushItems(te.getCapability(Capabilities.CAP_ITEM_HANDLER, side), target, maxItems = 10)
    }
  }
}
